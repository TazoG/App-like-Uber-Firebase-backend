...
class HomeController: UIViewController {
    ...
    private var trip: Trip? {
        didSet {
            ...
            if user.accountType == .driver {
                ...
                print("TAZO: Show ride action view for accepted trip")
            }
        }
    }
    ...
    override func viewWillAppear(_ animated: Bool) {
        guard let trip = trip else { return }
        print("TAZO: trip state is \(trip.state)")
    }
    ...
    func observeCurrentTrip() {
        PassengerService.shared.observeCurrentTrip { trip in
            ...
            case .denied:
                ...
                self.presentAlertController(withTitle: "Oops", message: "It looks like we could not find you a driver. Please try again later")
...
private extension HomeController {
    ...
    func zoomForActiveTrip(withDriverUid uid: String) {
        ...
        print("TAZO: Annotations array is \(annotations)")
...
extension HomeController: CLLocationManagerDelegate {
    func locationManager(_ manager: CLLocationManager, didStartMonitoringFor region: CLRegion) {
        if region.identifier == AnnotationType.pickup.rawValue {
            print("TAZO: Did start monitoring pickup regions \(region)")
        } else {
...
extension HomeController: UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        let selectedPlacemarks = indexPath.section == 0 ? savedLocations[indexPath.row] : searchResults[indexPath.row]
        ...
        dismissLocationView { _ in
            self.mapView.addAnnotationAndSelect(forCoordinate: selectedPlacemarks.coordinate)
            
            let annotations = self.mapView.annotations.filter({ !$0.isKind(of: DriverAnnotation.self) })
            self.mapView.zoomToFit(annotations: annotations)
            
            self.animateRideActionView(shouldShow: true, destination: selectedPlacemarks, config: .requestRide)
        }
    }
}
